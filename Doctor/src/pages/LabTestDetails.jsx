...
import { getLabTestDetails } from "@/services/labtestApi";
...
const LabTestDetails = () => {
  ...
  const { labTestDetails, loading, error } = useSelector((state) => state.labtest);

  useEffect(() => {
    dispatch(getLabTestDetails(labtestid));
  }, [dispatch, labtestid]);
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[60vh] bg-muted">
        ...
        <p className="text-muted-foreground font-medium">Loading lab report details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[60vh] bg-muted flex items-center justify-center p-4">
        <Alert variant="destructive" className="max-w-md">
          ...
          <AlertDescription>{error?.message || "Failed to load lab report"}</AlertDescription>
        </Alert>
      </div>
    );
  }

  if (!labTestDetails) return null;

  const { tests, overall_status, report_date, verified_by, verified_at, createdAt } = labTestDetails;

  return (
    <div className="min-h-screen doctor-page-gradient py-12">
      <div className="container mx-auto px-4 lg:px-8 max-w-4xl">
        <Button variant="ghost" onClick={() => navigate("/labtests")} className="mb-6 gap-2">
          ...
          <CardHeader className="space-y-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-muted-foreground">Review test status and result details</p>
              </div>
                      ...
                      <CardContent className="space-y-4">
                                ...
                                <tbody>
                                  {test.parameters.map((param, paramIndex) => (
                                    <tr key={paramIndex} className="border-b">
                                      ...
                                      <td className="p-2 text-muted-foreground">{param.reference_range}</td>
                                      <td className="p-2">
                                        <Badge
                                          variant={
                                            param.status === "Normal"
                        ...
                        {test.result_summary && (
                          <div className="p-3 bg-primary/5 rounded-lg">
                            ...
                            <p className="text-foreground">{test.result_summary}</p>
                          </div>
                        )}
                        ...
                        {test.remarks && (
                          <div className="p-3 bg-yellow-50 rounded-lg">
                            ...
                            <p className="text-foreground">{test.remarks}</p>
                          </div>
                        )}
                      </CardContent>
                    </Card>
